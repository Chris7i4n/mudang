...
### Fixed
- **Rust generic parameters no longer emit `references_type` edges.** Tree-sitter exposes generic param declarations and concrete type references both as `type_identifier` nodes, so the edge query was emitting one edge per occurrence of single-letter generics (`T`, `F`, `R`, `S`, `U`, `K`, `E`, `B`, `V`). Surfaced while indexing tokio in Sprint 12 — these accounted for ~24% of `references_type` edges (487 out of 1999). `extract_rust_edge` now filters via `is_likely_generic_param` (single uppercase ASCII letter). Regression test: `tests/integration/test_rust_lang.rs::test_rust_generic_params_do_not_emit_type_refs`.

### Notes
- Sprint 12 — Rust phase concluded against tokio-1.52.3. Confidence: **medium**. Two fix-now (FK ordering in v0.10.1, generic param leak in v0.10.2). Two recall gaps deferred: `cfg_*! { ... }` macro body extraction (defer-future, requires macro expansion) and `macro_rules!` defs (defer-doc, limite v1 declarado). See `docs/sprint-12/rust/README.md` for full audit.

## v0.10.1 (2026-05-08)

### Fixed
- **Rust indexing**: `impl Foo { ... }` blocks that appear earlier in a file than the `struct Foo;` declaration no longer abort indexing with `FOREIGN KEY constraint failed`. Tree-sitter emits captures in document order, so methods were inserted before their target type; `insert_file_data` now orders parents (no `parent_id`) before children. Surfaced while indexing tokio in Sprint 12. Regression test: `tests/integration/test_rust_lang.rs::test_rust_impl_before_struct_does_not_break_index`.

## v0.10.0 (2026-05-08)

### New Features
- **Ruby language support** — tree-sitter grammar integrated. Extracts classes, modules, instance methods, singleton methods, and constants. Edges cover `require`/`require_relative` imports, method calls, class inheritance, `include`/`prepend`/`extend` mixins (as `implements`), and structural constant references. Symbol metadata captures visibility (`public`/`private`/`protected`), singleton status, parameter list, and leading `#` doc comments. Conservative literal-only handling for `send`/`public_send`/`__send__`/`define_method`/`const_get`. Integration tests added under `tests/integration/test_ruby.rs` with fixtures in `tests/fixtures/ruby-simple/`.

### Changed
- **Tree-sitter core** bumped from `0.22.6` to `0.25.10` (constraint: `0.25.8`). Resolves growing distance from upstream and unlocks language-crate bumps below.
- **Language parsers** updated to current upstream releases:
  ...
  - `tree-sitter-ruby` 0.21.0 → 0.23.1
- **Plugin API migration** — all language plugins now use the `LANGUAGE_*` constants (`LanguageFn`, from `tree-sitter-language ^0.1`) instead of the deprecated `language()` function form.
- **Ruby v1 — Rails real-world audit refinements (Sprint 10).** No change to v1 contract; targeted fixes only.
  - `references_type` now also captures `Klass[key]` (`element_reference`) and `case x when Klass` (`when pattern`).
  - `extends` edge now attaches to the subclass itself rather than the enclosing module/scope (parser scope-anchor fix).
  - Module/interface sketch now renders `deps:`, `extends:`, and `implements:` lines (previously omitted).
  - `implements` list deduplicated when the same module is mixed in multiple times (`include X; prepend X`).
  - Symbol resolution prefers `lib/`, `src/`, and `app/` paths over `test/`, `spec/`, and `test/dummy/` when multiple definitions share a name.

### Internal
- **`QueryMatches` → `StreamingIterator`** in `src/core/parser.rs`. Tree-sitter 0.25 dropped `Iterator` for `QueryMatches` and now returns a reused reference per match via `StreamingIterator` (avoids per-match allocation, gives a more correct lifetime). Symbol- and edge-extraction loops converted from `for m in matches` to `while let Some(m) = matches.next()`. Added dependency `streaming-iterator = "0.1"`. No architectural change — same capture logic, same schema, ~7-line diff.

### Performance
- **`scope index --full`**: roughly **+13%** slower on the scope-cli repo itself (Apple M1, hyperfine, 3 runs: 2.86s → 3.24s). Newer grammars are richer (more node kinds, more rules) and parsing is heavier per file. Trade-off accepted — see Notes. Detail in `docs/dev-sessions-sprint-11/wrap/PERF-REPORT.md`.

### Quality
- **Ruby recovery improved on real Rails codebases** with no `.scm` change. Smoke run on a Rails monolith (2848 files, 40,388 symbols): edges grew from 176,668 to 269,114 (**+52%**), symbol count unchanged. Grammar 0.23 exposes more `call`/`command`/`block` AST that existing query patterns already covered, so DSL calls (`belongs_to`, `validates`, `scope`, `has_many`, etc.) and chained calls in blocks now reach the index. Detail in `docs/dev-sessions-sprint-11/ruby/RAILS-SMOKE-REPORT.md`.
- **TypeScript test suite** — dedicated `tests/integration/test_typescript.rs` added (12 tests covering classes, interfaces, type aliases, enums, methods, variants, and edges including `this.method()` calls and chained member calls). TypeScript was previously only covered indirectly by language-agnostic tests.

### Known Limitations (Ruby v1)
- **No type inference for Ruby** — constants tracked via `references_type` are structural references, not resolved class identities.
- **No semantic awareness of Rails, RSpec, ActiveRecord, Sorbet, Zeitwerk, RBS, RBI, or YARD** — DSLs are treated as ordinary Ruby calls.
- **No dynamic metaprogramming resolution** — only literal-argument forms of `send`/`public_send`/`__send__`/`define_method`/`const_get` are resolved. Variable, interpolated, or runtime-computed names are skipped.
- **DSLs registered via dynamic `define_method`** (e.g. `before_action`, `after_action`) are not indexed; `scope refs` reports "Symbol not found".
- **Cross-class membership without ancestor resolution** — subclass calls to mixin DSLs are not linked to `def`s in ancestor modules; partial recall.
- **Symbol lookup is FQN-only** — short names (`Concern`) do not suggest FQN candidates (`ActiveSupport::Concern`).
- **Concern `included do` block** — `include X` inside the block attaches to the textual enclosing scope, not the eventual includer.
- **Index build counts vs stored counts may differ** — `scope index` build output is pre-storage; `scope status` / `scope map` reflect stored (deduped/filtered) counts.
- **Sketch `deps:` mixes kinds** — constant `references_type` and call expressions are listed together rather than split.
- **Short-name symbol collisions** — `scope refs :initialize` and similar may surface incidental matches across files.
- **Language detection from lockfiles only** — `scope init` may report a language detected solely from a lockfile even when no source files of that language exist; `scope map --json` is authoritative.

### Documentation
- **Ruby v1 limitations section in `README.md`** expanded with the limits confirmed by the Rails real-world audit (Sprint 10).

### Notes
- New language syntax now supported via the updated parsers (non-exhaustive):
  - Python: PEP 695 type params, refined decorators.
  - TypeScript: `satisfies`, `using` declarations.
  - Rust: `let-else`, `async fn` in traits with more precise coverage.
  - Go: range-over-func (Go 1.23), generic type aliases.
  - Java: switch pattern matching, sealed classes.
  - C#: primary constructors, collection expressions (C# 12).
  - Ruby: `case/in` pattern matching, refined endless methods.
- No language was pinned. All seven language test suites passed without `.scm` adjustments (TS 12, Python 16, Rust 32, Go 17, Java 28, C# 13, Ruby 65 = 183 per-language tests; full suite 405 passed in release).
- **Trade-off rationale**: tree-sitter must be updated periodically to track language evolution. Pinning indefinitely is not viable. Output quality (recovery, correctness, modern-syntax coverage) takes precedence over raw parse throughput. Parser-side perf optimization is out of scope for this release and may be revisited in a dedicated sprint.
