...
[![Version](https://img.shields.io/badge/version-v0.10.0-blue.svg)](https://github.com/rynhardt-potgieter/scope/releases)
...
![Ruby](https://img.shields.io/badge/Ruby-ready-22863a?style=flat-square&logo=ruby&logoColor=white)

All seven languages have full support: tree-sitter grammar integration, symbol extraction, edge detection (calls, imports, extends, implements), and enriched metadata. C# includes partial class merging; Python includes decorator and docstring extraction; Rust includes visibility modifiers; Go includes receiver metadata; Java includes annotations and access modifiers; Ruby includes mixins, singleton methods, and visibility modifiers. TypeScript, C#, Java, and Rust also extract enum variants as a dedicated `variant` symbol kind with parent linking.

### Ruby support

Conservative structural analysis based on tree-sitter. Scope extracts:

- **Symbols:** classes, modules, instance methods, singleton methods, constants.
- **Edges:** `require`/`require_relative` (imports), method `calls`, class `inheritance` (extends), `include`/`prepend`/`extend` mixins (implements), constant `references_type`.
- **Metadata:** visibility (`public`/`private`/`protected`), singleton status, parameter list, leading `#` doc comments.

**Known Ruby v1 limitations.**

- No type inference; no runtime metaprogramming resolution.
- No semantic awareness of Rails, RSpec, ActiveRecord, Sorbet, Zeitwerk, RBS, RBI, or YARD — these DSLs are treated as ordinary Ruby calls.
- Dynamic metaprogramming only resolved for literal names: `send(:foo)`, `public_send("foo")`, `__send__`, `define_method` with literal name, `const_get` with literal name. Anything built from a variable, interpolation, or runtime value is skipped.
- `references_type` for a constant means a structural reference to that constant name, not proof that the constant resolves to a local class.
- DSLs registered via dynamic `define_method` (e.g. `before_action`, `after_action`) are not indexed as symbols; `scope refs` reports "Symbol not found" rather than distinguishing this from missing usage.
- Cross-class membership without ancestor chain resolution: subclass calls to mixin DSLs (e.g. `has_many` in classes inheriting `ActiveRecord::Base`) are not linked to the `def` in the ancestor module. Recall on these is partial.
- Symbol lookup requires fully-qualified names; short names (`Concern`) do not auto-suggest FQN candidates (`ActiveSupport::Concern`).
- `include X` inside a Concern's `included do` block creates an edge attached to the textual enclosing scope, not the eventual includer.
- File-count and edge-count between `scope index` build output and `scope status` / `scope map` may differ (build counts are pre-storage; stored counts reflect dedup/filter). Treat the latter as authoritative.
- Sketch `deps:` line currently merges constant `references_type` and call expressions in one list; not split by edge kind.
- `scope refs` may emit incidental matches when a short symbol name (e.g. `:initialize`) collides across files; treat low-volume entries as advisory.
- Language detection in `scope init` may report extra languages (e.g. TypeScript via JS lockfile only) even when no source files of that language exist; `scope map --json .data.stats.languages` is authoritative.
...
- [x] Rust language support -- structs, enums, traits, visibility modifiers
