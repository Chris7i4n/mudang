...
fn symbol_display_name(symbol: &Symbol) -> String {
    if let Some(parent_id) = &symbol.parent_id {
        if let Some(parent_name) = parent_id.split("::").nth(1) {
            return format!("{parent_name}.{}", symbol.name);
        }
    }
    ...
    match all.len() {
        ...
        _ => {
            ...
            for (i, s) in all.iter().enumerate() {
                let display_name = symbol_display_name(s);
                msg.push_str(&format!(
                    "  {}. {} ({})  {}:{}\n",
                    i + 1,
                    display_name,
            ...
            let display_name = symbol_display_name(&all[0]);
            if display_name != all[0].name {
                msg.push_str(&format!(
                    "\nUse a qualified name to disambiguate:\n  scope <cmd> {display_name}\n\
                     Or use the exact symbol id:\n  scope <cmd> {}::{}::{}",
                    all[0].file_path, all[0].name, all[0].kind,
                ));
            } else {
                msg.push_str(&format!(
                    "\nUse the exact symbol id to disambiguate:\n  scope <cmd> {}::{}::{}",
                    all[0].file_path, all[0].name, all[0].kind,
                ));
            }
            anyhow::bail!("{msg}");
