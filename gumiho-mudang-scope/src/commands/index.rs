...
mod tests {
    ...
    #[test]
    fn test_get_supported_extensions_ruby() {
        let config = ProjectConfig::default_for("ruby-test", vec!["ruby".to_string()]);
        let extensions = get_supported_extensions(&config);
        assert_eq!(extensions, vec!["rb".to_string()]);
    }
}
