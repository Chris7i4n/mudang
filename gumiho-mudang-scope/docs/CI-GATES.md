...
| lang_version coverage | R8 (sprint 0003 (d)) | Integration test `test_audit_confidence_lang_version_coverage_all_seven_languages` in `gumiho-mudang-cli/tests/integration/test_audit_confidence.rs` runs `mudang init + index --full + audit confidence --emit-sample` against one canonical fixture per supported language (`tests/fixtures/{rust,go,python,typescript,java,csharp,ruby}-simple/`) and asserts every emitted JSONL record carries the manifest-declared `lang_version` value (non-`null`, no drift). The test IS the gate — wired into the existing `audit-confidence` recipe (no separate recipe). | any fixture's audit emit produces a `null` `lang_version`, a value drifted from the manifest declaration, or zero sampleable edges — signals an unwired dispatcher arm / reader returning `None` for a shape it should handle / missing manifest in the fixture | `just audit-confidence` | active |
| edge_audit_history-source-immutability | R8 (sprint 0004 (j)) | `scripts/audit_history_immutability.sh` — narrow-grep gate. Two checks: (1) `gumiho-mudang-cli/src/commands/audit.rs` contains zero source-derived SQL-write keywords (`INSERT INTO` / `UPDATE` / `DELETE FROM` / `REPLACE INTO`, including `INSERT OR REPLACE INTO` and `INSERT OR IGNORE INTO`) paired with `edges` / `symbols` / `file_hashes` table names (word-boundary anchored so `edge_audit_history`, `edges_view`, etc. are excluded automatically); (2) `Graph::append_audit_history` body in `gumiho-mudang-scope/scope-graph/src/graph.rs` (extracted via awk brace-balance) contains no SQL string targeting those three source-derived tables. The function is the **sole DB write reachable from the `--label` flow**; source-derived tables (`edges`, `symbols`, `file_hashes`) stay frozen during audit per the writable-namespace carveout ([`AUDIT-LABEL-SCHEMA.md` § Writable namespace for audit-derived rows](AUDIT-LABEL-SCHEMA.md#writable-namespace-for-audit-derived-rows)). | any source-derived SQL mutation in `audit.rs`; any source-derived SQL mutation inside `append_audit_history`'s body; the `append_audit_history` function being removed or renamed (the gate cannot scope its body if the named anchor disappears) | `just ci-history-immutable` | active |
| Labeller-workspace isolation | R14 (sprint 0005 (b₁)) | `scripts/gate_labeller_isolation.sh` — narrow-grep gate. Three checks: (1) **root-cargo-toml-excludes-labeller** — root `Cargo.toml` declares `exclude = [..., "gumiho-mudang-labeller", ...]` under `[workspace]`; (2) **scope-side-path-dep-on-labeller** — no Scope-side `Cargo.toml` (root + `gumiho-mudang-{scope,cli,lsp}/**`) declares a `path = "..."` dependency pointing into `gumiho-mudang-labeller/`; (3) **labeller-side-path-dep-on-scope** — no `gumiho-mudang-labeller/**/Cargo.toml` declares a `path = "..."` dependency pointing into `gumiho-mudang-scope`, `gumiho-mudang-cli`, or `gumiho-mudang-lsp`. The boundary is the build-system fact that prevents labeller dependencies (LLM provider SDKs, `tower-lsp`, async HTTP runtimes — sprints 0010/0011/0012) from entering the Scope binary's `Cargo.lock`. | root `Cargo.toml` missing the exclude entry; any cargo path-dep edge across the workspace boundary in either direction | `just gate-labeller-isolation` | active |
| Doc-sync | R13 | `scripts/gate_doc_sync.sh` — narrow-grep gate. Initial checks (sprint 0001): **enforcement-map-paths** (every backticked file path cited in `ENFORCEMENT-MAP.md` resolves on disk), **ci-gates-recipes** (every `active` row's `just <recipe>` exists in `justfile`), **doc-relative-links** (every relative markdown link under `gumiho-mudang-scope/docs/` resolves), **cycle-docs-indexed** (`SELF-CORRECTION-CYCLE.md` + `SELF-CORRECTION-STATE.md` referenced from docs `README.md` when present). Per-sprint refinements: **audit-samples-layout** (sprint 0002 / (e)) checks every `LanguageId::as_str` slug owns a `reference/<slug>/audit-samples/` directory, no extras; **lang-version-detector-modules** (sprint 0003 / (d)) checks every `LanguageId` variant has both a `detect_in_dir` arm in `lang_version.rs` and a `### <Display>` subsection under `CHARTER.md § 7`; **schema-version-const-doc** (sprint 0004 / (g)) checks `SAMPLE_SCHEMA_VERSION` const value ↔ both the `## Record schema (\`schema_version: "X"\`)` heading and the table-row `Always "X"` description in `AUDIT-LABEL-SCHEMA.md`; **sample-record-fields** (sprint 0004 / (g)) checks every `pub <name>:` field on `SampleRecord` has a `\`<name>\`` row in the record-schema table (one-way ⊆); **coverage-summary-fields** (sprint 0004 / (h)) checks the `CoverageSummary` struct field set equals the parenthesised field list in `ENFORCEMENT-MAP.md` § R8's `coverage_summary` bullet (bidirectional); **edge-audit-history-columns** (sprint 0004 / (j)) checks the `CREATE TABLE IF NOT EXISTS edge_audit_history (…)` column set in `scope-graph/src/sql/schema.sql` equals the column list inside `ENFORCEMENT-MAP.md` § R0's "Audit-derived rows (writable namespace)" bullet's backticked CREATE-TABLE span (bidirectional). Extension protocol for later Priority 1 sprints: one new `check_<short_name>` function per drift shape, per [`SELF-CORRECTION-CYCLE.md` § Extending the doc-sync gate](SELF-CORRECTION-CYCLE.md#extending-the-doc-sync-gate). | any check finds a drift hit (named doc value ≠ named code value, or named doc surface points to a non-existent path) | `just gate-doc-sync` | active |
...
ci-gates: ci-trait-shape ci-no-spawn ci-no-network ci-immutable ci-context-shape ci-no-fs ci-dispatch ci-edge-sealed ci-no-framework-scm ci-patterns ci-output-schema test-builder test-typestate test-malformed audit-confidence ci-history-immutable
