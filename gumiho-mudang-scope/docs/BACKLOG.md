...
- **(b) Reference labeller crates.** External-to-Scope crates that implement the JSONL contract: `scope-audit-labeller-llm` (provider-agnostic LLM wrapper), `scope-audit-labeller-lsp` (per-language LSP cross-check via `tower-lsp` clients), `scope-audit-labeller-hybrid` (LLM-first, human-reviews-diffs). They consume only the schema published in [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md).

  **Physical location.** Sibling directory `gumiho-mudang-labeller/` at the repo root with its own `[workspace]` block, listed under the root workspace's `[workspace] exclude = [...]`. Two cargo workspaces in one repo, isolated build artefacts and lock files. Labeller deps (LLM provider SDKs, `tower-lsp`, async runtimes for HTTP) never enter the root `Cargo.lock` and never enter the Scope binary. Resolves the "separate workspace" ambiguity in favour of in-repo isolation over a separate git repo — single-operator hygiene keeps both surfaces under one history while cargo enforces the dependency boundary mechanically. The sibling directory is the boundary that turns CHARTER §3 invariant 6 ("Deterministic, read-only at query time. No network calls") and §5 hard limit ("Network calls during query — Breaks determinism, sandbox compatibility, offline use") into a build-system fact, not just a discipline.

  **Sprint split.** Each concrete labeller is its own atomic sprint; the workspace itself is scaffolded once and then the three labellers ship independently:
  - **(b₁) Workspace scaffolding + shared `scope-audit-labeller-core` crate** (sprint 0005). Sibling workspace `gumiho-mudang-labeller/` exists with the root-exclude wired, the shared core crate defines the `Labeller` trait + v2 JSONL read/write helpers consuming [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md) only, and a no-op `scope-audit-labeller-noop` reference impl proves the trait + IO loop end-to-end. None of the concrete labellers ship in this sprint.
  - **(b₂) `scope-audit-labeller-llm`** (sprint 0010). Provider-agnostic LLM wrapper. Ships after (b₁) lands and the trait shape is stable.
  - **(b₃) `scope-audit-labeller-lsp`** (sprint 0011). Per-language LSP cross-check via `tower-lsp` clients. Independent of (b₂).
  - **(b₄) `scope-audit-labeller-hybrid`** (sprint 0012). LLM-first + human-reviews-diffs orchestration. Composes over (b₂) once shipped; can dev-spike against (b₁)'s noop while (b₂) is in flight.

  The three concrete labellers do not block each other and can be sequenced or parallelised by other constraints. (b₁) blocks all three because the trait + IO contract is its deliverable.
...
- **(g) Richer auditor verdict types — JSONL `schema_version` bump `"1"` → `"2"`.** The current JSONL sample-row is pinned at `schema_version: "1"` with a binary `label: bool | null` verdict. That surface is **minimum-viable for the plug-point only**: a binary correct/wrong/skipped is information-poor signal — a labeller saying "wrong" tells you nothing about *why* it is wrong, and "skipped" tells you nothing about *what* was actually true. The actionable signal for closing the self-correction loop is **qualitative**: the auditor saying *"Scope claimed X, here is the evidence that Y is the truth"* — proposed corrected target, proposed corrected kind, proposed corrected confidence tier, free-text reasoning, cross-check evidence. The schema-bump-2 record adds (each `null` on emit, populated by capable labellers; partial population tolerated):
...
- **(i) Multi-labeller verdict aggregation.** Realistic labelling pipelines run several labellers in series or parallel (LSP fast-path for `calls`, LLM for everything else, human reviewer for diffs) and produce conflicting verdicts on the same `edge_id`. Today there is no aggregation surface — each `--label` invocation consumes one file. (i) designs:
  ...
  - **Read-side views absorbed from (j)** — sprint 0004 deferred two `scope audit history` views to this sub-item because they are sparse under a single-operator labelling pipeline:
    - **`scope audit history labeller <id>`** — chronological label timeline for one `labeller_id` (all edges that labeller weighed in on, across audits).
    - **`scope audit history agreement-matrix`** — NxN matrix where cell `(i, j)` = percentage of `(edge_id, audit_id)` tuples where labeller `i` and labeller `j` produced the same `label`. Drives the disagreement-diagnostics signal this sub-item is centrally concerned with.

- **(j) Audit-history persistence in the DB — separate writable namespace, auditor immutability preserved.** Today `--label` consumes a JSONL file and emits a stdout report; nothing of the audit result flows back to `graph.db`. That keeps the auditor-immutability rule absolute *for the source-derived tables* (`edges`, `symbols`, `file_hashes`) but means **historical audit data is not queryable**. A pattern that fails precision every Tuesday for three months produces three months of independent reports — Scope itself cannot say *"this pattern_id has trended downward over N audits"*. (j) designs the persistence layer:
  ...
  - **Auditor immutability rule extension** — [`AUDIT-LABEL-SCHEMA.md` § Auditor immutability rule](AUDIT-LABEL-SCHEMA.md#auditor-immutability-rule) gets a new paragraph carving out the writable namespace explicitly so the read-only-on-source promise remains mechanically checkable.
  - **`scope audit history`** subcommand — read-side surface for the new table. Becomes the input for (f) (the patch suggester operates on history, not just a single audit run). Sprint 0004 scope is constrained by the single-operator posture (CHARTER §3 invariant 1) — a single human labeller renders matrix-style multi-labeller views sparse-to-empty until (i) ships. The surface delivered in sprint 0004:
    ...
    - **Deferred to sprint 0006** (sub-item (i), where multi-labeller density makes them meaningful): `scope audit history labeller <id>` (per-labeller timeline) and `scope audit history agreement-matrix` (NxN cross-labeller agreement). Listed here so the surface is not lost; sprint 0006 picks them up under (i)'s deliverables.
...
- **Stable cross-session symbol IDs** — `scope-core/src/parser.rs` carries the implementation; maintained, not re-planned.
