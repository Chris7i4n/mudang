...
**Indexer-side carveout** — the workspace readers in `scope-core/src/workspace/*.rs` are not subject to C2: they live indexer-side, behind the trait boundary. A reader may expose a version-extraction free function (e.g. `extract_edition(content)`, `extract_go_directive(content)`, `extract_tsconfig_target(content)`) for indexer-side consumers — the R8 audit emit's `lang_version` field is the canonical consumer (`BACKLOG.md` § Priority 1 sub-item (d), shipped in sprint 0003). Language plugins cannot reach these functions because plugins receive the trait, not the reader module; the trait surface remains the C2 enforcement point gated by `audit_context_shape.sh`. The rule's intent — *plugin extraction never branches on language version* — is preserved; the readers merely surface the version string for the auditor.
...
6. **Build 5+ real-world fixtures.** Same discipline as frameworks — anonymized snippets from real maintainer projects, not synthetic toy code. Fixtures land under `scope-core/tests/fixtures/reference/<lang>/` and feed the R8 confidence audit subcommand ([`ENFORCEMENT-MAP.md` § R8](ENFORCEMENT-MAP.md#r8--confidence-audit-subcommand)). Anonymization rules are non-negotiable — see [`scope-core/tests/fixtures/reference/README.md` § Anonymization rules](../scope-core/tests/fixtures/reference/README.md#anonymization-rules). The malformed-source corpus (R6) has different provenance rules (hand-crafted synthetic, not anonymized) and lives under `scope-core/tests/fixtures/malformed/<lang>/` — see [`scope-core/tests/fixtures/malformed/README.md`](../scope-core/tests/fixtures/malformed/README.md).
