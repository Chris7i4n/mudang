...
- [ ] Dispatch table is single-sourced in the indexer; the only inputs
      are `(extension, shebang)`.
- [ ] Each plugin declares `accepts_extension(&str) -> bool` and
      `accepts_shebang(&str) -> bool`. No other opt-in mechanism exists.
...
1. **R5 trait surface depends on `FrameworkWorkspaceContext` but R5
   ships later (sprint 0005, Phase C).** This sprint lands the
   `FrameworkWorkspaceContext` trait so R5 has it; whether the trait is
   exposed publicly before any `FrameworkPlugin` impl exists is an open
   question. Two readings:
   - **Land both context traits in this sprint, both public.** R5 then
     consumes `FrameworkWorkspaceContext` directly. Risk: a public trait
     with no implementor invites premature use.
   - **Land both context traits in this sprint, framework one
     `pub(crate)`.** R5 widens visibility when it lands. Risk: API
     churn between Phase B close and Phase C open.

   Halt and consult the human; the resolution is committed to
   `ARCHITECTURAL-REFACTOR.md` R4's "Target state" on `main` **before**
   this sprint's branch opens.

2. **R7 plugin trait signatures.** `accepts_extension` and
   `accepts_shebang` are new trait methods. Their addition touches every
   language plugin in `src/languages/*`. Whether they replace the
   existing extension-list pattern (`fn extensions(&self) -> &[&str]`)
   wholesale or coexist for one release cycle is not specified in R7.
   Halt and consult; resolution amends `ARCHITECTURAL-REFACTOR.md` R7
   "Target state".
...
2. The two ambiguities above are resolved before code lands; the
   resolution is committed to `ARCHITECTURAL-REFACTOR.md` on `main`
   before this sprint's branch opens.
...
5. Every existing language plugin (`src/languages/rust.rs`, `python.rs`,
   `go.rs`, `typescript.rs`, `java.rs`, `csharp.rs`, `ruby.rs`)
   compiles against the new context traits and the new dispatch hooks,
   with zero direct filesystem access.
