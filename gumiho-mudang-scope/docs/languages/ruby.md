...
- **C2** (no version-specific compiler-quirk modelling): **mechanically enforced on the plugin-facing trait surface** — no Ruby-version accessor in `LanguageWorkspaceContext`, pinned by `audit_context_shape.sh`. The `.ruby-version` reader and the `Gemfile` `ruby` directive reader added in sprint 0003 live **indexer-side** behind that trait boundary; per the R4 indexer-side carveout they may expose Ruby-version extraction functions that indexer-side consumers — the R8 audit emit's `lang_version` field, shipped in sprint 0003 (d) — call directly. Plugins never reach those functions; the C2 line stays at the trait, not at the reader.
