...
- **Durable contract**:
  ...
  - **Audit-derived rows (writable namespace)**: `edge_audit_history (audit_id INTEGER NOT NULL, edge_id INTEGER NOT NULL, labelled_at INTEGER NOT NULL, labeller_id TEXT, label TEXT NOT NULL CHECK (label IN ('correct','incorrect','skipped')), target_proposed TEXT, kind_proposed TEXT, confidence_proposed TEXT, evidence_json TEXT)` — append-only, no primary key (audit log permits duplicates), no foreign key on `edge_id` (history outlives source). Indices: `idx_edge_audit_history_edge_audit(edge_id, audit_id)` + `idx_edge_audit_history_labeller_audit(labeller_id, audit_id)`. The table is the **only** writable surface during `scope audit confidence --label`; source-derived tables (`edges`, `symbols`, `file_hashes`) stay frozen. The carveout is mechanically enforced by [R8](#r8--confidence-audit-subcommand)'s `edge_audit_history-source-immutability` gate; the schema doc records the carveout under [`AUDIT-LABEL-SCHEMA.md` § Writable namespace for audit-derived rows](AUDIT-LABEL-SCHEMA.md#writable-namespace-for-audit-derived-rows).
