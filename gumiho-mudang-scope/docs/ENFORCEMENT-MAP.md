...
- **Durable contract**:
  ...
  - **Audit-derived rows (writable namespace)**: `edge_audit_history (audit_id INTEGER NOT NULL, edge_id INTEGER NOT NULL, pattern_id TEXT NOT NULL, from_id TEXT NOT NULL, to_id TEXT NOT NULL, labelled_at INTEGER NOT NULL, labeller_id TEXT, label TEXT NOT NULL CHECK (label IN ('correct','incorrect','skipped')), target_proposed TEXT, kind_proposed TEXT, confidence_proposed TEXT, evidence_json TEXT)` — append-only, no primary key (audit log permits duplicates), no foreign key on `edge_id` (history outlives source). `pattern_id` / `from_id` / `to_id` are denormalised onto the audit row so both halves of the pattern drill (precision timeline + `currently_incorrect` drivers) are robust under wipe-and-reindex (`edges.edge_id` is AUTOINCREMENT and is not stable across reindex; the source-derived pattern_id slug and the symbol identifiers are). The denormalisation is the canonical durable form of "history outlives source": no read query against this table ever JOINs `edges`. Indices: `idx_edge_audit_history_edge_audit(edge_id, audit_id)` + `idx_edge_audit_history_labeller_audit(labeller_id, audit_id)` + `idx_edge_audit_history_pattern_audit(pattern_id, audit_id)`. The table is the **only** writable surface during `scope audit confidence --label`; source-derived tables (`edges`, `symbols`, `file_hashes`) stay frozen. The carveout is mechanically enforced by [R8](#r8--confidence-audit-subcommand)'s `edge_audit_history-source-immutability` gate; the schema doc records the carveout under [`AUDIT-LABEL-SCHEMA.md` § Writable namespace for audit-derived rows](AUDIT-LABEL-SCHEMA.md#writable-namespace-for-audit-derived-rows).
...
- **Durable contract**:
  ...
  - **Indexer-side carveout**: workspace readers may expose **free functions** that return version-coupled values (e.g. `extract_edition(content)`, `extract_go_directive(content)`, `extract_tsconfig_target(content)`) for indexer-side consumers — the R8 audit emit's `lang_version` field is the canonical consumer ([`BACKLOG.md` § Priority 1 sub-item (d)](BACKLOG.md#priority-1--self-correction-cycle), shipped in sprint 0003). Plugins cannot reach these functions because plugins receive the `LanguageWorkspaceContext` trait, not the reader module; the trait surface remains the C2 enforcement point. The carveout's scope is exactly: a reader may surface the version string for the auditor; plugins still must not branch on language version.
...
- **Durable contract**:
  ...
  - **Auditor immutability**: hard mechanical SHA-256 drift gate via `Graph::check_audit_freshness` (no `--allow-drift` escape). The auditor never mutates source-derived tables (`edges`, `symbols`, `file_hashes`).
  - **Writable namespace for audit-derived rows** (sprint 0004 (j)): `edge_audit_history` is the **only** table `scope audit confidence --label` may write to. `Graph::append_audit_history` is the sole writer reachable from the `--label` flow; it allocates `audit_id = COALESCE(MAX(audit_id), 0) + 1` inside the writing transaction (race-free under single-operator posture) and writes every row with a shared `labelled_at` UNIX timestamp. The carveout is mechanically enforced by the `edge_audit_history-source-immutability` gate (`scripts/audit_history_immutability.sh`) — two narrow-grep checks: (1) `gumiho-mudang-cli/src/commands/audit.rs` contains zero source-derived SQL-write keywords against `edges` / `symbols` / `file_hashes`; (2) the body of `Graph::append_audit_history` (extracted via awk brace-balance) contains no SQL string targeting those three source-derived tables. Removing or renaming `append_audit_history` fails the gate (the anchor disappears). [R0](#r0--schema-closures) records the table schema; [`AUDIT-LABEL-SCHEMA.md` § Writable namespace for audit-derived rows](AUDIT-LABEL-SCHEMA.md#writable-namespace-for-audit-derived-rows) carries the prose carveout.
  - **Output format**: default `--format json` carrying `schema_version`, `disclaimer`, `sample_schema_doc`, `coverage_summary`, `report[]`. `--format tsv` is a convenience view. `report[]` rows carry per-group coverage fields (`labelled_count`, `skipped_count`, `coverage_ratio`, `precision: Option<f64>`); `coverage_summary` carries top-level coverage (`records_total`, `records_labelled`, `records_skipped`, `distinct_groups_with_coverage`, `distinct_groups_fully_skipped`).
  - **JSONL sample schema** (full doc in [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md)): `schema_version: "2"`, fields `edge_id`, `kind`, `confidence`, `producer`, `pattern_id`, `from`, `to`, `source_snippet`, `lang_version`, `label`, plus the seven v2 qualitative fields `evidence`, `target_proposed`, `kind_proposed`, `confidence_proposed`, `reasoning_text`, `lang_version_evidence`, `labeller_id` (each `null` on emit, populated by capable labellers; partial population tolerated). The bump landed in sprint 0004 (g) as hard-cutover — `--label` rejects every `schema_version` other than the current `SAMPLE_SCHEMA_VERSION` const with the same "unknown schema_version" diagnostic that fires on a forward bump (CHARTER § 3 invariant 1: single-operator posture, no dual-read shim).
  - **Read-side surface** (sprint 0004 (j)): `scope audit history` ships three views over `edge_audit_history` — default aggregate dashboard (headline + top-N regressing patterns + top-N flapping edges), `scope audit history edge <id>` (chronological label timeline for one edge), `scope audit history pattern <id>` (precision-over-time for one pattern + currently-`incorrect` drivers). Deferred to sprint 0006 (i) under single-operator-posture reasoning: `scope audit history labeller <id>` and `scope audit history agreement-matrix`.
...
- **Where in the tree**: `gumiho-mudang-cli/src/commands/audit.rs` (subcommand surface — extraction to `scope-audit` sub-crate queued in [`BACKLOG.md` § Priority 3 — Layering audit](BACKLOG.md#priority-3--layering-audit-thin-cli-fat-library)); `gumiho-mudang-scope/scope-graph/src/graph.rs` (`AuditEdgeRow`, `AuditFreshness`, `check_audit_freshness`, `AuditHistoryRow`, `append_audit_history`, `audit_history_edge`, `audit_history_pattern`, `audit_history_dashboard`); `scope-graph/src/sql/schema.sql` (`edge_audit_history` table + indices); `scope-core/tests/fixtures/reference/<lang>/audit-samples/` (committed labelled corpus).
- **CI gates**: Confidence audit (`just audit-confidence` runs the integration suite); edge_audit_history-source-immutability (`just ci-history-immutable` runs `scripts/audit_history_immutability.sh`).
...
- **Durable contract**: `scripts/gate_doc_sync.sh` — narrow-grep gate. Initial checks shipped in sprint 0001:
  ...
  - **schema-version-const-doc** (sprint 0004 / (g)) — `SAMPLE_SCHEMA_VERSION` const value in `gumiho-mudang-cli/src/commands/audit.rs` matches both the `## Record schema (\`schema_version: "X"\`)` heading and the `schema_version` field's `Always "X"` description cell in [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md). The schema version is the wire contract for external labellers — heading / cell / const must all agree.
  - **sample-record-fields** (sprint 0004 / (g)) — every `pub <name>:` field inside `pub struct SampleRecord { … }` has a matching `\`<name>\`` row in the [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md) record-schema table. One-way (code ⊆ doc): documenting more than is implemented is forward-reservation; adding a struct field without a doc row leaks an undocumented wire field.
  - **coverage-summary-fields** (sprint 0004 / (h)) — `pub struct CoverageSummary { … }` field set equals the parenthesised field list in this R8 entry's `coverage_summary` bullet. Bidirectional: code-side rename without doc edit drifts; doc cites unknown field name also drifts.
  - **edge-audit-history-columns** (sprint 0004 / (j)) — `CREATE TABLE IF NOT EXISTS edge_audit_history (...)` column set in `gumiho-mudang-scope/scope-graph/src/sql/schema.sql` equals the column list inside the [R0](#r0--schema-closures) "Audit-derived rows (writable namespace)" bullet's backticked CREATE-TABLE span. Bidirectional.
