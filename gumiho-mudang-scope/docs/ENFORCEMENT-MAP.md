...
- **Durable contract**:
  ...
  - **Audit-derived rows (writable namespace)**: `edge_audit_history (audit_id INTEGER NOT NULL, edge_id INTEGER NOT NULL, pattern_id TEXT NOT NULL, labelled_at INTEGER NOT NULL, labeller_id TEXT, label TEXT NOT NULL CHECK (label IN ('correct','incorrect','skipped')), target_proposed TEXT, kind_proposed TEXT, confidence_proposed TEXT, evidence_json TEXT)` — append-only, no primary key (audit log permits duplicates), no foreign key on `edge_id` (history outlives source). `pattern_id` is denormalised onto the audit row so the pattern-drill read surface (`scope audit history pattern <id>`) is robust under wipe-and-reindex (`edges.edge_id` is AUTOINCREMENT and is not stable across reindex; the pattern_id slug is). Indices: `idx_edge_audit_history_edge_audit(edge_id, audit_id)` + `idx_edge_audit_history_labeller_audit(labeller_id, audit_id)` + `idx_edge_audit_history_pattern_audit(pattern_id, audit_id)`. The table is the **only** writable surface during `scope audit confidence --label`; source-derived tables (`edges`, `symbols`, `file_hashes`) stay frozen. The carveout is mechanically enforced by [R8](#r8--confidence-audit-subcommand)'s `edge_audit_history-source-immutability` gate; the schema doc records the carveout under [`AUDIT-LABEL-SCHEMA.md` § Writable namespace for audit-derived rows](AUDIT-LABEL-SCHEMA.md#writable-namespace-for-audit-derived-rows).
