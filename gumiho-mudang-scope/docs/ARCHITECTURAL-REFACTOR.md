...
- **Target state**:
  ...
  - **Provenance**:
    ...
    - `edges.args_text TEXT NULL` — call-site or declaration-site argument literal as written in source, capped at 2 KB. Populated for edge kinds whose anchor information lives in the arguments (HTTP routes, queue enqueues, env reads, GraphQL operations, pubsub topics, etc.). Two mitigations bound the cost:
      ...
      - **Mitigation 2**: when a literal exceeds the 2 KB cap, the column stores the first 2 KB followed by the marker `[truncated]`. Audits (R8) treat truncated rows as candidates for confidence downgrade. **Slated for removal post-refactor**: the cap is justified by *"common case fits"*, not by a hard runtime constraint (panic / OOM / won't-run), and therefore violates the honesty principle locked in [`POST-REFACTOR-PLAN.md` § Priority 2 — Honesty audit](POST-REFACTOR-PLAN.md#priority-2-immediately-post-refactor--honesty-audit-eliminate-non-essential-approximations). Priority 2 sub-item (c) drops it; until then R8 sidesteps the issue by reading `source_snippet` from the source file at audit time, not from `args_text`.
...
- **Target state**: plugin returns `RawCaptures` — a typed bag of `.scm` capture results, declared metadata (decorator args, type annotations as text, etc.), **and an explicit `skipped_ranges` field** for regions the plugin chose to skip (e.g., a macro body it cannot interpret). The indexer concatenates the plugin-reported skips with its own tree-sitter-error skips before writing `file_hashes.skipped_ranges` (R6). A separate `Extractor` layer converts `RawCaptures` to `Edge::builder()` calls. The conversion layer applies confidence rules consistently and is the only place that knows about `EdgeKind`. Plugin authors cannot directly emit edges; they cannot directly write confidence values. The extraction interface has no method whose name implies inference, expansion, resolution, or evaluation.
  ...
  **Extractor location (sprint 0003 resolution).** The `Extractor` layer lives in `scope-core/src/extract/`. Rationale: the extractor is a pure transform (`RawCaptures` → `RawEdge` via `EdgeBuilder`) with zero I/O, knowledge of `EdgeKind` is `scope-core` domain (R0 whitelist), and co-location with the plugin layer (`scope-core/src/languages/`) keeps trait visibility internal — no cross-crate widening of `RawCaptures` / `EdgeBuilder` field visibility. `scope-index/src/indexer.rs` stays a thin orchestrator that calls `plugin → extract → resolve → write` in sequence. The R12 (sprint 0004) process-spawn denylist path filter includes `scope-core/src/extract/` for this reason.
  ```rust
...
- **Target state**: pipeline is `extract → resolve → write`, encoded in the type system.
  ...
  - **Multi-row Ambiguous — scope's producer commitment (sprint 0003 resolution).** When the resolver finds N candidate targets for a given `RawEdge`, it emits **N rows** of `InsertableEdge` — one per candidate — each with `status = Ambiguous`, the same `(from_id, kind, source_position)`, and `to_id` set to the candidate. The candidate set is preserved on disk as evidence; the resolver does not invent a tiebreak. Charter-grade rationale: (1) R8 audit precision math depends on per-`(producer, pattern_id)` ambiguity counts that single-row collapse would hide; (2) D2 "no best-guess fallback" forbids the resolver from picking a winner without semantic information; (3) the LSP-enrichment pipeline (background composer, `SCOPE-LSP-COMPOSITION.md`) requires the candidate set so semantic resolution can confirm or supersede individual rows without inventing context. Collapsing at the producer is a charter-amendment-grade decision, not a routine PR.
  ...
  **Resolver location (sprint 0003 resolution).** The resolver lives in `scope-graph/src/resolve/`. Rationale: resolution needs `Graph` read access (consult `symbols` for `to_id` lookup) which is `scope-graph`'s domain; it consumes `LanguageWorkspaceContext` (R4) for workspace-internal disambiguation; it must be the **sole** constructor of `InsertableEdge`, so module-level encapsulation in the same crate that owns `Insertable` keeps the typestate compile-error airtight (`pub(crate)` ctor in `scope-graph::resolve` is unreachable from `scope-core` and `scope-index`). `scope-core` cannot host the resolver (no `Graph` access); `scope-index` cannot (would force `InsertableEdge` constructor to widen `pub`, breaking R3 typestate). The R12 (sprint 0004) process-spawn denylist path filter includes `scope-graph/src/resolve/` for this reason.
...
- **Target state**: plugins receive a typed context, never raw paths. **The context is split into two traits** so that a language plugin cannot accidentally read fields that would tempt a C2 violation, while a framework plugin can read framework version because branching on it is its job (R5):
  ...
  **Visibility of `FrameworkWorkspaceContext` (R4 → R5 handoff).** R4 (sprint 0002, Phase B) lands `LanguageWorkspaceContext` as `pub` and `FrameworkWorkspaceContext` as `pub(crate)` in `scope-core`. The framework context trait stays `pub(crate)` for the entirety of Phase B (sprints 0002, 0003, 0004) so that the Rust compiler refuses any language-plugin code that tries to import or bound on it — the mechanical extension of the split-trait safeguard. R5 (sprint 0005, Phase C) widens `FrameworkWorkspaceContext` to `pub` in the **same commit** that lands the first `FrameworkPlugin` impl. The visibility flip is not a stub (it is legitimate visibility, not a placeholder); it is not tracked in `REFACTOR-STATUS.md` § Stubs outstanding. Sprint 0005's ambiguity register records the flip as mandatory, not conditional.
...
- **Target state — model B (eager metadata)**:
  ...
  - **Trait shape**:
    ...
    struct Detection {
        ...
        applies_to_languages: Vec<LanguageId>, // variants of `scope-core/src/languages/id.rs::LanguageId` (post-R7 rename of `SupportedLanguage`); Rails → [Ruby]; React → [TypeScript]
    }
  ...
  - **Synthetic test framework location** (sprint 0005 ambiguity #1 resolution): R5 lands the infrastructure but does not adopt any real framework. The integration-test suite exercises the trait against a **synthetic** framework declared at `gumiho-mudang-scope/scope-core/tests/synthetic_framework/mod.rs`. The synthetic framework lives under `scope-core/tests/`, not under `scope-core/src/frameworks/`, for two reasons: (1) it must never reach the production binary — `src/`-side `#[cfg(test)]` gating is not enough because release builds still walk the module tree for documentation generation; (2) Rust integration tests compile as a separate crate, so a `src/`-side `#[cfg(test)] pub(crate)` module is unreachable from `tests/*.rs` regardless. Integration tests pull the synthetic in via Rust's shared-module convention (`mod synthetic_framework;` from each `tests/*.rs` that needs it). The synthetic's fixtures live at `scope-core/tests/fixtures/frameworks/synthetic/`, version-pinned subtrees per acceptance bullet.
  - **Cross-language fixture location** (sprint 0005 ambiguity #2 resolution): the pre-filter integration-test fixture lives at `gumiho-mudang-scope/scope-core/tests/fixtures/frameworks/_pre_filter/` with one subtree per participating language (`python/`, `ruby/`). The underscore prefix denotes "test infrastructure, not a real-world framework fixture" — the same convention as `_test` modules. Per-real-framework fixtures (when frameworks adopt post-refactor) sit under `scope-core/src/frameworks/<name>/fixtures/`, the layout already specified above; the `_pre_filter/` corpus is the R5-infrastructure-only counterpart.
  - **JavaScript coverage in R5 acceptance** (sprint 0005 ambiguity #4 resolution): R5 acceptance requires synthetic-framework coverage over the seven currently-supported `LanguageId` variants (`TypeScript, CSharp, Python, Go, Java, Rust, Ruby`) only. JavaScript-shaped integration tests are deferred to post-refactor per [`POST-REFACTOR-PLAN.md` § Items deliberately deferred → `.js`/`.jsx` indexing](POST-REFACTOR-PLAN.md#items-deliberately-deferred-beyond-this-plan). The cheap path (extending `LanguageId::TypeScript.extensions()` to include `js|jsx`) and strict path (new `JavaScript` variant) remain post-refactor decisions; this sprint does not pre-commit to either.
...
- **Acceptance**:
  ...
  - Integration test: a fixture with a Python file containing a decorator name that matches a Ruby framework's predicate produces zero edges from that framework plugin (cross-language pre-filter is honored). Fixture location: `scope-core/tests/fixtures/frameworks/_pre_filter/{python,ruby}/`.
  ...
  - JavaScript (`.js` / `.jsx`) coverage: deferred to post-refactor per `POST-REFACTOR-PLAN.md` § Items deliberately deferred. R5 acceptance is satisfied by synthetic-framework coverage over the seven `LanguageId` variants extant at sprint 0005 cut (`TypeScript, CSharp, Python, Go, Java, Rust, Ruby`).
...
- **Target state**:
  ...
  - **Indexer behavior**: when tree-sitter recovery encounters an `ERROR` or `MISSING` node region, the parser records the region's line range and reason into `skipped_ranges` for the file. The reason families emitted by the implementation (per `scope-core/src/extract/error_scan.rs` + sprint 0008 chunk 3 audit) are exactly: `tree_sitter_error:syntax_error` (ERROR node), `tree_sitter_error:missing_node` (MISSING node), and `plugin_skip:<plugin_name>:<rationale>` (plugin-driven skip — sprint 0003 chunk 3a wiring). Tree-sitter exposes only `is_error()` and `is_missing()` accessors, so there is no separate "unrecoverable" reason kind; a MISSING node is the closest analog and is already captured by `tree_sitter_error:missing_node`. A plugin that deliberately skips a sub-tree (e.g., a macro body it cannot interpret) appends its own range via the third family.
...
- **Enforces**: F4.
- **Current state**: file-to-language dispatch is split across two places — `scope-core/src/parser.rs::detect_language` holds a hardcoded `match ext { "py" => Python, ... }` while each `LanguagePlugin` impl also declares `fn extensions(&self) -> &[&str]`. Two opt-in mechanisms; the same knowledge in two places; plugin can theoretically inspect file content to opt in. Additionally, the workspace carries a parallel pair of types — `enum SupportedLanguage` (identifier) + 7 unit structs (`PythonPlugin`, `RustPlugin`, …) implementing `trait LanguagePlugin`. The struct-and-enum pair admits a representable-but-incorrect state: `(SupportedLanguage::Python, &TypeScriptPlugin)` typechecks; only the `find_plugin` convention prevents the mismatch.
- **Target state — compile-time dispatch, no runtime election, no trait, no per-language unit struct**:
  - The `enum LanguageId` (the rename of the historical `SupportedLanguage`) is the **single** type that names a language. There is no parallel struct hierarchy; per-language behaviour lives in `impl LanguageId` match arms that delegate to per-language modules (`languages::python::extract_metadata`, etc.). The `trait LanguagePlugin` is **removed**; the seven `PythonPlugin` / `RustPlugin` / … unit structs are **removed**.
    ```rust
    // scope-core/src/languages/id.rs
    #[derive(Copy, Clone, Debug, PartialEq, Eq, Hash)]
    pub enum LanguageId {
        TypeScript, CSharp, Python, Go, Java, Rust, Ruby,
    }

    impl LanguageId {
        pub const fn as_str(self) -> &'static str { /* "typescript", "csharp", ... */ }
        pub const fn extensions(self) -> &'static [&'static str] { /* match arms */ }
        pub const fn shebangs(self)   -> &'static [&'static str] { /* match arms */ }
        pub fn ts_language(self) -> tree_sitter::Language { /* match arms */ }
        pub fn symbol_query_source(self) -> &'static str { /* match arms */ }
        pub fn edge_query_source(self)   -> &'static str { /* match arms */ }
        pub fn extract_metadata(self, node, source, kind, ctx) -> Result<String> { /* match arms */ }
        pub fn extract_edge(self, pattern_index, captures, file_path, scope, ctx) -> Vec<Edge> { /* match arms */ }
        pub fn extract_docstring(self, node, source) -> Option<String> { /* match arms */ }
        pub fn infer_symbol_kind(self, node_kind) -> &'static str { /* match arms */ }
        pub fn scope_node_types(self) -> &'static [&'static str] { /* match arms */ }
        pub fn class_body_node_types(self) -> &'static [&'static str] { /* match arms */ }
        pub fn class_decl_node_types(self) -> &'static [&'static str] { /* match arms */ }
        pub const fn generic_name_stopwords(self) -> &'static [&'static str] { /* match arms */ }
    }
    ```
    Every method is **exhaustive over `LanguageId`**. Adding a variant without adding an arm = compile error. The router file is large (~250 lines of match arms) but contains no logic — only per-language module delegation; the per-language module files retain their existing extraction logic verbatim.
  - **No representable invalid state.** Without a separate plugin instance, `(LanguageId, &PluginStruct)` mismatch is unrepresentable: the `LanguageId` variant **is** the dispatch target. The struct-side of the historical pair is deleted entirely.
  - Dispatch lives in `scope-core/src/languages/dispatch.rs`, generated by a declarative macro `register_languages!` over `LanguageId` variants:
    ```rust
    register_languages! {
        TypeScript, CSharp, Python, Go, Java, Rust, Ruby,
    }
    ...
    - `pub const fn dispatch_extension(ext: &str) -> Option<LanguageId>` — sequence of `const_slice_contains(LanguageId::<V>.extensions(), ext)` checks; first match wins.
    - `pub const fn dispatch_shebang(line: &str) -> Option<LanguageId>` — same shape over `LanguageId::<V>.shebangs()`.
    - A `const _: () = assert_no_extension_overlap(&[...]);` block — `const fn` that panics at compile time if two variants claim the same extension.
  - **No `KNOWN_EXTENSIONS` static** — the candidate set is the union of `LanguageId::<V>.extensions()` consts, evaluated by the macro at expansion time. Single source per variant.
  - `parser.rs::detect_language` hardcoded match is removed; callers go through `dispatch_extension`.
  - Indexer is the only consumer of `dispatch_extension` / `dispatch_shebang`; once it has a `LanguageId`, all extraction calls are methods on the enum.
- **Migration**:
  - **B.3 (enum rename)** — `enum SupportedLanguage` renamed to `LanguageId`; module home moves from `parser.rs` to `scope-core/src/languages/id.rs`. `SupportedLanguage::as_str()` becomes `LanguageId::as_str()`; serialized strings (`"typescript"`, `"csharp"`, `"python"`, `"go"`, `"java"`, `"rust"`, `"ruby"`) are **preserved verbatim** — database `symbols.language` and `edges.producer` text columns continue to round-trip the same slugs.
  - **A.4 (trait collapse)** — `trait LanguagePlugin` and the seven `*Plugin` unit structs are deleted. Their methods migrate to `impl LanguageId` match arms; per-language module functions stay where they are.
  - **C.1 (stopwords signature)** — `pub fn stopwords_for_language(language: &str)` is deleted. Callers use `lang.generic_name_stopwords()` directly on the `LanguageId`. The stringly-typed lookup with silent `_ => &[]` fallback is closed by exhaustive match.
  - `parser.rs::detect_language` removed; `parser.rs::CodeParser::is_supported` routed through `dispatch::dispatch_extension`.
  - `register_languages!` macro and `const_panic` uniqueness assertion land in `scope-core/src/languages/dispatch.rs`.
- **Acceptance**:
  - `trait LanguagePlugin` does not exist anywhere in the workspace.
  - No `*Plugin` unit struct exists (`grep -r 'struct \(.*\)Plugin\b' scope-core/src/languages/` returns nothing).
  - `enum SupportedLanguage` does not exist; `enum LanguageId` is the single language-identifier type.
  - Every method on `LanguageId` is implemented with an **exhaustive match** — adding a variant without adding arms fails the build.
  - `register_languages!` macro is the only call site that names every `LanguageId` variant together for dispatch generation.
  - A `const _: () = assert_no_extension_overlap(...)` block in `dispatch.rs` fails the build if any extension is claimed by two variants.
  - `dispatch_extension` / `dispatch_shebang` are `const fn`; `parser.rs::detect_language` is removed.
  - `stopwords_for_language(&str)` is removed; callers use `LanguageId::generic_name_stopwords()`.
  - Database `symbols.language` text values are byte-identical to pre-R7 (no migration; `as_str` preserves slugs).
  - Indexer dispatches only via `dispatch_extension` / `dispatch_shebang`; no language code reads file content or path to self-activate (CI grep gate: `just ci-dispatch`).
...
- **Operational shape** (Phase D pre-branch ambiguity resolutions — recorded here before Phase D's first sprint branch opens; R8 ships in sprint 0007 after sprint 0006 lands R10):
  - **Reference fixture corpus** (Phase D pre-branch ambiguity #1 resolution): the corpus lives at `gumiho-mudang-scope/scope-core/tests/fixtures/reference/<language_slug>/`, one subtree per supported `LanguageId`. Provenance: real anonymized snippets from the maintainer's projects (per [`LANGUAGE-PLAYBOOK.md` Step 5](LANGUAGE-PLAYBOOK.md#step-5--implementation-procedure-within-bounds) → "real-world fixtures, not synthetic toy code"). Anonymization rules — strip secrets (tokens, hostnames, credentials), replace proprietary identifiers with shape-preserving placeholders that retain length, casing, and any framework-conventional prefixes/suffixes that affect predicate matching — are recorded in `tests/fixtures/reference/README.md` (authored alongside the first fixture). The corpus is committed and version-controlled; updating any fixture re-anchors the precision baseline for the next R8 run.
  - **Sampling protocol** (Phase D pre-branch ambiguity #2 resolution): default `N = 30` edges per `(kind, confidence)` combination. `--sample-size N` flag overrides per invocation. Sampling is seeded for reproducibility: `--seed N` (default a fixed compile-time constant). The 30-default is a binomial-proportion lower bound — at p=0.95 it yields a ±18-percentage-point half-width at 95% CI, adequate for early tier-drift signal; the maintainer tightens with `--sample-size` once tier-drift suspicion exists.
  - **Labelling channel** (Phase D pre-branch ambiguity #3 resolution — extended sprint 0007 to lock the plug-point contract): manual labelling is the default channel R8 ships, but the **JSONL sample file is the contract for any external labeller** (LLM, LSP cross-check, hybrid human-in-the-loop). Two-phase workflow: (1) `scope audit confidence --emit-sample <path>` writes an unlabelled, seed-pinned JSONL sample to `<path>` — one edge per line, with a `label: null` slot; (2) **anything** fills the `label` slot — the maintainer by hand, a script wrapping an LLM API, a script wrapping an LSP server's `go-to-definition` for cross-check, or a hybrid pipeline where an LLM labels first and a human reviews diffs; (3) `scope audit confidence --label <path>` reads the labelled file and produces the precision report. Scope itself spawns no subprocess and runs no labeller; the file format is the boundary. A separate flag like `--labeller <executable>` is **not** part of R8 — adding subprocess plumbing for a single labeller class would weaken the contract that any external pipeline (offline, networked, batch, interactive) can plug in. The full JSONL schema (every field, semver rules, external-labeller examples for LLM and LSP) is documented in [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md). Active labeller crates (`scope-audit-labeller-llm`, `scope-audit-labeller-lsp`, etc.) are post-refactor work and live in [`POST-REFACTOR-PLAN.md` § Priority 1 — Self-correction cycle](POST-REFACTOR-PLAN.md#priority-1-immediately-post-refactor--self-correction-cycle).
  - **Output format** (Phase D pre-branch ambiguity #4 resolution — extended sprint 0007 to lock the sample-row schema): default `--format json`. The JSON document carries a top-level `schema_version: "1"` field plus a `report` array of `(kind, tier, producer, pattern_id, sample_size, correct_count, precision)` rows. `--format tsv` is also supported for shell pipelines and emits one row per `(kind, tier, producer, pattern_id)` combination with the same columns. The JSON shape is the contract; TSV is a convenience view of the same row set. Adding a new column is a schema bump (`schema_version: "2"`); removing one is a charter-grade change. **JSONL sample schema** (consumed by `--emit-sample` → `--label`): each line is a JSON object with fields `edge_id`, `kind`, `confidence`, `producer`, `pattern_id`, `from`, `to`, `source_snippet`, `lang_version: Option<String>`, `label: Option<bool>` — the `lang_version` slot is reserved for per-project language version (e.g. Rust edition, Go directive, Python `requires-python`, TS `tsconfig.target`, Java source compatibility, C# TargetFramework, Ruby `.ruby-version`); sprint 0007 ships this slot **always `null`** because per-language detector wiring is a separate atomic delivery (see [`POST-REFACTOR-PLAN.md` § Priority 1 sub-item (d)](POST-REFACTOR-PLAN.md#priority-1-immediately-post-refactor--self-correction-cycle)). Schema is locked at `schema_version: "1"`; populating `lang_version` is forward-compatible (existing labelled samples remain valid; `null` is a legal value). Full schema documentation, semver rules, and external labeller examples live in [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md).
...
- **Sprint 0006 scope decision** (mid-sprint amendment, recorded before R10 implementation begins): the sprint 0006 plan named three explicit output structs (`SymbolSketch`, `EdgeSummary`, `CompactView`) plus "any others currently driven by raw string concatenation". The R10 target-state phrase "Formatters serialize structs; they do not concatenate strings" is read **strictly**: every output renderer in `gumiho-mudang-cli` — including the per-command surfaces (`refs`, `deps`, `impact`, `trace`, `flow`, `entrypoints`, `status`, `workspace`, `map`, `find`, `similar`, `source`, `diff`) and the `--json` envelope — converts to a `#[derive(Serialize)]` struct or enum at the output boundary. Procedural `println!`/`format!` collapses into `impl Display` on the typed output struct. `serde_json::json!()` ad-hoc tree construction is removed from the codebase; every JSON-emitting code path serializes a concrete typed value. The decision rests on three considerations: (1) the "make illegal states unrepresentable" Rust idiom — sum types over render variants prevent constructing a `ClassSketch`-shaped output for a method-shaped symbol, mechanically blocking entire bug classes that B/C (lighter-touch interpretations) cannot; (2) `#[derive(Serialize)]` streams to the writer with no intermediate `Value` tree (whereas `json!()` allocates `serde_json::Value` BTreeMap + Vec before serializing), so the strict interpretation is both safer and faster in the release-fast profile after LTO inlines the monomorphized Serialize impls; (3) the typed-output surface is the prerequisite for future cross-tool consumers (e.g., the LSP composition layer reading Scope output without string parsing — captured in [`gumiho-mudang-lsp/docs/SCOPE_OUTPUT_INTEROP.md`](../../gumiho-mudang-lsp/docs/SCOPE_OUTPUT_INTEROP.md) and `SCOPE-LSP-COMPOSITION.md` § 5.4). The cost — ~3-5 days work, all 28 `insta` snapshot tests re-anchor, ~50 KB extra binary size from serde-derive monomorphization — is accepted. Scope was confirmed mid-sprint; this amendment locks the strict reading so codex review and future sprints reference the same contract.
