# Architectural refactor — closure record

The architectural refactor closed on **2026-05-12**. R0–R12 across phases A–E shipped; every CI gate listed in [`CI-GATES.md`](CI-GATES.md) is `active` and runs under `just gate-refactor`.

This document is the **durable record** of the closure: which charter / playbook rules the architecture mechanically prevents or detects, which R-move owns each rule, and where the enforcement lives in the tree. The plan-shape sprints / status-log / acceptance-gate prose that drove the refactor while it was in flight lived in `sprints/0000…0009-*.md` and `REFACTOR-STATUS.md`; those files were deleted at refactor close. Their history is preserved in git (search for the `refactor/sprint-*` branch merges and the `chore(refactor-status): …` commits on `main`).

Companion to `CHARTER.md`, `LANGUAGE-PLAYBOOK.md`, `FRAMEWORK-PLAYBOOK.md`. The charter defines what Scope is. The playbooks define the rules every language and framework plugin respects. This document explains how the architecture itself enforces — or auto-detects — those rules.
...
Class 3 is reserved for rules that genuinely require human judgment. The class-3 universal list is short and explicit: **B1, C2, E3** (see § What remains discipline-only).

---

## Inventory of constraints — where each rule lives

This table maps every universal rule to its enforcement class and the R-move + audit that closes it. The R-move column points into § Refactor moves below; the audit column points into [`CI-GATES.md`](CI-GATES.md).

### Charter hard limits (charter section 5)

| Rule | Class | Closure |
|---|---|---|
| No compiler/interpreter invocation | detectable | R12 process-spawn denylist (`scripts/audit_no_spawn.sh`) |
| No live type inference | detectable | R12 trait-shape lint (`scripts/audit_trait_shape.sh`) + R8 audit |
| No macro/template/preprocessor expansion | detectable | R11 trait-shape lint + R8 audit |
| No editor-buffer state | mechanical | filesystem-only indexer; no LSP-style mutable buffer protocol exists |
| No network at query time | detectable | R12 network-denylist gate (`scripts/audit_no_network.sh`) |
| No generic instantiation tracking | detectable | R12 + R8 |
| No trait-bound checking | detectable | R12 + R8 |
| No lifetime / borrow analysis | detectable | R12 + R8 |
| No reflection / dynamic dispatch resolution | detectable | R12 + R8 |
| No conditional-type evaluation | detectable | R12 + R8 |
| No metaclass / monkey-patching resolution | detectable | R12 + R8 |
| No semantic rename refactor | mechanical | no write path in plugin layer |
| No type errors / borrow errors / lint diagnostics | mechanical | R10 — typed output schema rejects diagnostic-shaped fields |

### 18 language-plugin boundaries (LANGUAGE-PLAYBOOK Step 4)

| Rule | Class | Closure |
|---|---|---|
| A1 no type inference | detectable | R12 lint + R8 audit |
| A2 no constraint solving | detectable | R12 lint + R8 audit |
| A3 no type-system name resolution | detectable | R12 lint + R8 audit |
| B1 no flow analysis | discipline | judgment-bound; R8 catches symptoms |
| B2 no runtime / dynamic resolution | detectable | R12 lint + R8 audit |
| B3 no assumption of valid syntax | detectable | R6 — malformed-source harness + `skipped_ranges` schema |
| C1 no macro/template expansion | detectable | R11 trait-shape lint |
| C2 no version-specific quirks | discipline | judgment-bound; R4 closes the leakage path |
| D1 no cross-file resolution beyond config | mechanical | R4 — `LanguageWorkspaceContext` is the only path |
| D2 no best-guess fallback | mechanical | R0 + R1 + R3 — confidence/status required at insertion, resolution mandatory |
| D3 no collision resolution by guessing | mechanical | R0 surrogate PK + `status='ambiguous'` representable; R1 forces explicit confidence |
| E1 no semantic correctness assertions | mechanical | R10 — output schema has no diagnostic fields |
| E2 no metadata interpretation in language plugin | mechanical | R5 — `FrameworkPlugin` trait shape forbids AST access |
| E3 no heuristic hot-path optimization | discipline | judgment-bound |
| F1 no multi-pass semantic analysis in plugin | mechanical | R3 — pipeline order via type-state |
| F2 no write-back to source | mechanical | R9 — immutable references everywhere |
| F3 no file-format parsing beyond tree-sitter | mechanical | R4 — config readers are the only path; plugin trait does not expose file IO |
| F4 no content sniffing | mechanical | R7 — indexer dispatches by extension+shebang via const-fn match |

### Why detectable, not mechanical, for trait-shape rules

A name-based audit (R12 trait-shape) catches a method called `infer_type_at`, but does not catch a helper method named `compute_X_for_Y` that performs the same work. The process-spawn denylist catches `Command::new("rustc")` literally but not `Command::new(env::var("CC")?)` — the binary name resolves at runtime. True mechanical enforcement would require module isolation (separate crate with no `tree_sitter` dep on the inference path), an explicit dependency denylist, or a sandboxed plugin runtime — feasible follow-ups parked in [`POST-REFACTOR-PLAN.md`](POST-REFACTOR-PLAN.md).

The closure is the **combination** of three layers: trait-shape audit, process-spawn denylist, and R8 confidence audit. The trait-shape and spawn audits catch sloppy implementations at PR time; the R8 audit catches the symptom (overconfident edges) when a clean-but-forbidden implementation evades both gates by being correctly named and dynamically invoked. **Honest framing**: a determined plugin author can still write a correctly-named helper and a runtime-resolved compiler call that R8 cannot easily reach if its precision is high — that residual surface is why the discipline-only universal list (B1, C2, E3) is short but not empty, and why the `detectable` label is best-effort rather than exhaustive. Detection is the gate, not prevention; rules listed as `detectable` are enforced by the combined audits **for typical violations**, with a small unobserved area for the determined-evasion case that falls back to code review.
...
Each move has: **ID, rules it enforces, durable contract, where to find it in the tree**. The R-move shape is preserved here as the durable index — external docs (charter, glossary, playbooks, post-refactor plan, audit-label schema) link into these section anchors.
...
- **Durable contract**:
  ...
  - **Provenance**:
    ...
    - `edges.pattern_id TEXT NOT NULL` — short slug naming the pattern that produced the edge (`calls.method`, `imports.use`, `http_route.decorator_literal`, …). Used by R8 to localize tier drift to a specific pattern.
  ...
  - **Symbols metadata structured fields** (JSON shape, no schema change): `decorators`, `annotations`, `template_calls` — reserved keys for framework consumption (R5). Other free-form keys may exist. Hooks-style detection is intentionally **not** a reserved metadata key; framework plugins apply regex matchers themselves at the framework layer (per R5).
...
- **Migration policy**: wipe-and-reindex (CHARTER § 2 + § 3 invariant 8). `rm -rf .scope/ && scope index` rebuilds from source.
...
- **Durable contract**:
  ...
  - `RawCaptures.skipped_ranges` is the **plugin-driven skip channel** for B3 / R6 (e.g. macro bodies the plugin cannot interpret). The indexer concatenates plugin-reported skips with its own tree-sitter-error skips before writing `file_hashes.skipped_ranges`.
  ...
  - **No method on the extraction interface implies inference, expansion, resolution, or evaluation** — names are caught by R12's trait-shape audit.
...
- **Durable contract** (model B — eager metadata):
  ...
  - **Language plugins populate `symbols.metadata` with structured primitives** (the three reserved keys: `decorators`, `annotations`, `template_calls` — schema in R0). They do not emit generic primitive edges (would pollute the graph for projects that use no framework). Metadata sits on the symbol; if no framework matches, no derived edge is created.
...
- **Where in the tree**: `gumiho-mudang-scope/scope-core/src/frameworks/` (trait + dispatch); `scope-core/tests/synthetic_framework/` (R5 acceptance fixture).
- **CI gates**: No framework SCM (`audit_no_framework_scm.sh`); Pattern catalog audit (`audit_patterns.sh`); R12 trait-shape audit extended to `frameworks/`.

### R6 — Malformed-source test harness

- **Enforces**: B3.
- **Durable contract**:
  - Schema field `file_hashes.skipped_ranges` (from R0) carries JSON array `[{start_line, end_line, reason}]`.
  ...
  - **Out of scope**: invalid UTF-8 at the byte level. Recording such files honestly is a separate refactor with its own trigger.
...
- **Durable contract** (compile-time dispatch, no runtime election, no trait, no per-language unit struct):
  ...
  - DB `symbols.language` and `edges.producer` text slugs preserved verbatim by `LanguageId::as_str()`; no schema migration vs. pre-R7.
...
- **Durable contract**:
  ...
  - **JSONL sample schema** (full doc in [`AUDIT-LABEL-SCHEMA.md`](AUDIT-LABEL-SCHEMA.md)): `schema_version: "1"`, fields `edge_id`, `kind`, `confidence`, `producer`, `pattern_id`, `from`, `to`, `source_snippet`, `lang_version`, `label`. `lang_version` is reserved-for-future; ships always `null`. Bump to `"2"` is scheduled in [`POST-REFACTOR-PLAN.md` § Priority 1 sub-item (g)](POST-REFACTOR-PLAN.md#priority-1-immediately-post-refactor--self-correction-cycle).
...
- **Enforces**: C1.
- **Durable contract**: no function named `expand_*` in `scope-core/src/languages/` or `scope-core/src/extract/`. Macro symbols index under `kind: macro` (definition only). Macro invocations land as `calls.macro` / `calls.macro.scoped` edges to the macro symbol — never expanded. The trait-collapse amendment (A.4, sprint 0002) means there is no `LanguagePlugin` trait to inspect; the negative shape is enforced over `impl LanguageId` arms + per-language modules + per-extractor modules instead, with the negative-trait-shape doc block on `id.rs` module header.
- **Where in the tree**: subset of R12 trait-shape audit (`expand_*` arm of `audit_trait_shape.sh`).
...
- **Honest limit**: trait-shape audit catches obvious names but not a helper named `compute_X_for_Y` that performs the same forbidden work. The process-spawn denylist catches literal `Command::new(` introductions but not a runtime-resolved `Command::new(env::var("CC")?)`. This is why A1–A3 + B2 + "no compiler/interpreter invocation" remain `detectable`, not `mechanical`. R8 is the symptom-side safety net.
- **Where in the tree**: `scripts/audit_trait_shape.sh`, `scripts/audit_no_spawn.sh`, `scripts/audit_no_network.sh`.

---

## Phase order (historical record)

The refactor shipped in five phases, each atomically (all moves in the phase landed together or none landed):

- **Phase A** (Schema and storage closures) — R0, R1. Shipped 2026-05-11.
- **Phase B** (Plugin layer closures) — R2, R3, R4, R7, R9, R11, R12. Shipped 2026-05-12 (atomic close via `refactor/phase-b` integration branch).
- **Phase C** (Framework layer closure) — R5. Shipped 2026-05-12.
- **Phase D** (Output and audit closures) — R8, R10. Shipped 2026-05-12 (atomic close via `refactor/phase-d` integration branch).
- **Phase E** (Test harness and charter close) — R6 + charter sweep (no R-move). Shipped 2026-05-12 (sprint 0008 R6; sprint 0009 charter sweep + shim retirement closed Phase E and the refactor as a whole).

The five-phase atomic-shipment shape is the durable template for any future scope-side initiative that needs to land architecture changes across multiple sprints. The methodology lives in [`sprints/README.md`](sprints/README.md) and the sprint skeleton in [`sprints/_TEMPLATE.md`](sprints/_TEMPLATE.md).

---

## What remains discipline-only

These three universal rules cannot be enforced by code; they require human judgment.

- **B1 (no flow analysis)** — flow analysis takes many syntactic forms; static detection of "is this plugin doing flow analysis" is itself an unbounded analysis problem. Safety net: code review against the explicit rule, plus R8 catches the symptoms (overconfident edges) when the root cause is missed.
- **C2 (no version-specific compiler-quirk modelling)** — distinguishing "syntactic capture" from "semantic interpretation tied to a language version" requires knowledge of the specific language. Safety net: code review against the explicit rule. R4 closes the **leakage path** mechanically (the trait surface omits language-version fields), so a plugin that wanted to drift into C2 cannot reach the inputs — but the rule itself (don't interpret version-specific semantics from the syntax tree) remains discipline-bound.
...
The 15 framework-gotcha categories from FRAMEWORK-PLAYBOOK Step 4 and the rule-temptation entries in language plugin docs are **per-instance decisions logged in the per-plugin templates** (`docs/frameworks/_TEMPLATE.md`, `docs/languages/_TEMPLATE.md`). They are not universal rules and they are not in any class of the inventory. The contract: every adopted plugin has a complete walkthrough table with explicit decisions. R5's trait shape and R0's metadata schema make those walkthroughs tractable but do not encode the decisions themselves — the decisions are framework-specific and live with the framework's doc.

---

## Where future feature work goes

The architecture is closed. Future feature work — per-language depth, framework rollout, vector embeddings, time-travel queries, `scope link`, `.js`/`.jsx` indexing, the self-correction cycle, the honesty audit, the layering audit — is queued in [`POST-REFACTOR-PLAN.md`](POST-REFACTOR-PLAN.md). Each item respects its own gate (language adoption flow per `LANGUAGE-PLAYBOOK.md`; framework adoption flow per `FRAMEWORK-PLAYBOOK.md`; trigger frequency for cross-cutting items).
