...
- **Target state** (single migration with schema-version bump via SQLite `PRAGMA user_version`, applied inside the migration transaction; current value is 0 because the legacy schema never set it; target value is 1 after R0 lands and increments per subsequent schema-affecting refactor):
  ...
  - **Edge kind whitelist additions**: `contains` (universal lexical containment, used by every language plugin) plus the domain kinds `http_route`, `queue_handler`, `orm_relation`, `goroutine_spawn`, `renders`, `hook_use`, `inherits_from`, `migration`, `cron`, `feature_flag`, `awaits_on`, `channel_send`, `channel_recv` (14 net-new). Combined with the existing 7 the final whitelist is 21. **No generic primitive edges** (`decorator_call`, `annotation_call`, `template_render`, `hook_call`) are added — primitives live in `symbols.metadata` per R5; only domain edges are top-level rows.
...
- **Migration**:
  - One migration script, atomic, wrapped in a transaction with schema-version bump.
  - SQLite `ALTER TABLE` cannot add NOT NULL columns without a default. Strategy: create new tables with the target schema, copy rows via `INSERT INTO new SELECT … FROM old` with conservative defaults (`confidence='low'`, `status='dangling'`, `producer='legacy_backfill'`, `pattern_id='legacy'`, `capture_id=NULL`), drop old, rename new. The same dance applies to `file_hashes` for `skipped_ranges`.
  - Edge `edge_id` rows get sequential IDs from `AUTOINCREMENT`.
  - **Backfill is conservative and does not depend on the R8 audit**. Re-indexing rewrites edges with their honest tiers as files are touched. The audit (R8, Phase D) measures tier accuracy after re-indexing has had time to run; it is not a precondition of R0.
- **Acceptance** (mechanical only — precision validation lives in R8 and is not a Phase A gate):
  - Schema migration applies cleanly to a legacy `.scope/` index produced by the pre-refactor binary; no rows are lost; new columns are populated with conservative defaults; `PRAGMA user_version` increments from 0 to 1.
  - `scope status` reports the current schema version (`schema_version: u32` on `StatusData`) and refuses to query an index whose `user_version` is newer than the binary supports.
...
- `scope status` reports the schema version (read from SQLite `PRAGMA user_version` and surfaced as `schema_version: u32` on `StatusData` in `src/commands/status.rs`) and refuses to operate against an index whose `user_version` is greater than the binary's compiled `EXPECTED_SCHEMA_VERSION`.
