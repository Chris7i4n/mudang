...
- **Enforces**: F4.
- **Current state**: file-to-language dispatch is mostly extension-based but not formalized as the only path. `scope-core/src/parser.rs::detect_language` holds a hardcoded `match ext { "py" => Python, ... }` while each `LanguagePlugin` also declares `fn extensions(&self) -> &[&str]`. Two opt-in mechanisms; the same knowledge in two places; plugin can theoretically inspect file content to opt in.
- **Target state — compile-time dispatch, no runtime election**:
  - Plugin capability is **associated constants** on each plugin type, not trait methods:
    ```rust
    impl PythonPlugin {
        pub const ID:         LanguageId        = LanguageId::Python;
        pub const EXTENSIONS: &'static [&'static str] = &["py", "pyi"];
        pub const SHEBANGS:   &'static [&'static str] = &["python"];
    }
    ```
    `LanguagePlugin` trait has no `accepts_extension` / `accepts_shebang` runtime methods. Capability is `const`; plugin instance never decides at runtime.
  - `LanguageId` is an enum exhaustive over supported languages (one variant per plugin). Adding a plugin without adding a variant fails to compile.
  - Dispatch lives in `scope-core/src/languages/dispatch.rs`, generated by a declarative macro `register_languages!` over the `LanguageId => PluginType` pairs:
    ```rust
    register_languages! {
        Python     => PythonPlugin,
        Rust       => RustPlugin,
    ...
    - `pub const fn dispatch_extension(ext: &str) -> Option<LanguageId>` — sequence of `const_slice_contains(<Plugin>::EXTENSIONS, ext)` checks; first match wins.
    - `pub const fn dispatch_shebang(line: &str) -> Option<LanguageId>` — same shape over `SHEBANGS`.
    - `pub fn plugin_for(id: LanguageId) -> &'static dyn LanguagePlugin` — exhaustive `match` over `LanguageId`. Compiler enforces every variant has an arm.
    - A `const _: () = assert_no_extension_overlap(&[...]);` block — `const fn` that panics at compile time if two plugins claim the same extension.
  - **No `KNOWN_EXTENSIONS` static** — the candidate set is the union of `<Plugin>::EXTENSIONS` consts, evaluated by the macro at expansion time. Single source per plugin.
  - **`extensions(&self) -> &[&str]` is removed** from the `LanguagePlugin` trait. `parser.rs::detect_language` hardcoded match is removed; callers go through `dispatch_extension`.
  - Indexer's dispatch site is the only consumer of `dispatch_extension` / `dispatch_shebang` / `plugin_for`.
- **Migration**: introduce `LanguageId` enum + `register_languages!` macro + `dispatch.rs`; migrate each of the seven plugins (`python.rs`, `rust_lang.rs`, `typescript.rs`, `go_lang.rs`, `java.rs`, `csharp.rs`, `ruby.rs`) to expose `ID` / `EXTENSIONS` / `SHEBANGS` associated consts and drop the `extensions()` method; remove `parser.rs::detect_language` and route callers through `dispatch.rs`; add the const-panic uniqueness assertion.
- **Acceptance**:
  - `LanguagePlugin` trait has no `extensions`, `accepts_extension`, `accepts_shebang`, or any other runtime opt-in method.
  - Every plugin type exposes `const ID`, `const EXTENSIONS`, `const SHEBANGS`.
  - `register_languages!` is the only call site that names the seven plugins together; `plugin_for(LanguageId)` is an exhaustive `match`.
  - A `const _: () = assert_no_extension_overlap(...)` block in `dispatch.rs` fails the build if any extension is claimed by two plugins.
  - Indexer dispatches only via `dispatch_extension` / `dispatch_shebang`; no plugin code reads file content or path to self-activate (CI grep gate: `just ci-dispatch`).
