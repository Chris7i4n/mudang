...
- **Operational shape** (sprint 0006 ambiguity resolutions — recorded here before sprint 0006 branch opens):
  ...
  - **Labelling channel** (sprint 0006 ambiguity #3 resolution): manual labelling is the default and the only channel R8 ships. Two-phase workflow: (1) `scope audit confidence --emit-sample <path>` writes an unlabelled, seed-pinned sample to `<path>`; (2) the maintainer fills the `label` slot per row (`correct` / `incorrect` / `unsure`); (3) `scope audit confidence --label <path>` reads the labelled file and produces the precision report. LLM-assisted labelling (`--labeller <executable>`) is explicitly deferred post-refactor: an LLM labeller is itself a precision source and demands its own audit; sprint 0006 ships only the manual loop. The deferral is recorded in [`POST-REFACTOR-PLAN.md` § Items deliberately deferred](POST-REFACTOR-PLAN.md#items-deliberately-deferred-beyond-this-plan).
  - **Output format** (sprint 0006 ambiguity #4 resolution): default `--format json`. The JSON document carries a top-level `schema_version: "1"` field plus a `report` array of `(kind, tier, producer, pattern_id, sample_size, correct_count, precision)` rows. `--format tsv` is also supported for shell pipelines and emits one row per `(kind, tier, producer, pattern_id)` combination with the same columns. The JSON shape is the contract; TSV is a convenience view of the same row set. Adding a new column is a schema bump (`schema_version: "2"`); removing one is a charter-grade change.
