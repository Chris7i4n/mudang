...
Mechanically: `Detection { detected, version, applies_to_languages }` (`ARCHITECTURAL-REFACTOR.md` R5) carries the version. The framework predicate inspects `Detection.version` and selects which `Symbol.metadata` shapes to match. The version source of truth is the workspace config: `Gemfile.lock`, `package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, exposed via `WorkspaceContext` (R4). The framework plugin never reads files directly.
...
**Allowed values are the variants of `SupportedLanguage`** (see `src/core/parser.rs:115`): `TypeScript`, `CSharp`, `Python`, `Go`, `Java`, `Rust`, `Ruby`. JavaScript is not currently a `SupportedLanguage` variant. **`.js` and `.jsx` files are not indexed today** — the indexer's extension dispatch (`src/core/parser.rs::detect_language`) accepts only `ts|tsx`, and the `TypeScriptPlugin::extensions()` declares only `&["ts", "tsx"]` (`src/languages/typescript.rs:22`). React-style framework plugins therefore declare `applies_to_languages = vec![SupportedLanguage::TypeScript]` and match exclusively against `.ts`/`.tsx` sources; a `.jsx` file written in plain JavaScript is invisible to every framework predicate today.

Adding `.js`/`.jsx` indexing requires one of two changes, each governed by its own playbook:

- **Cheap path** — extend `TypeScriptPlugin::extensions()` to `&["ts", "tsx", "js", "jsx"]` and add `"js" | "jsx"` arms to `detect_language`. `tree-sitter-typescript` parses both syntactically as a superset (`.tsx` grammar covers JSX). Type annotations in TS-only sources continue to populate `references_type` edges; JS sources skip them. This is a small `LANGUAGE-PLAYBOOK.md` Step 7 maintenance change to the existing TypeScript plugin and does not need a new SupportedLanguage variant.
- **Strict path** — add `JavaScript` as a separate `SupportedLanguage` variant with its own plugin. Earns extension-disambiguation (a `.tsx` file is unambiguously TS-with-JSX; a `.jsx` file is unambiguously JS-with-JSX) at the cost of grammar duplication and per-language fixture work. Triggered through `LANGUAGE-PLAYBOOK.md`'s adoption flow (Step 1 trigger threshold, Step 2 ROI worksheet).
...
2. **Pick the highest-frequency pattern first.** For Flask, that's the `@app.route` decorator. For React, that's JSX rendering. For Axum, that's the route macro. Implement the predicate (SQL or Rust matcher over `Symbol.metadata` and `Edge` rows) for just this pattern.
