...
| `labeller_id` | String identifier of the labeller that produced an audit verdict. Convention: `<kind>:<recipe>` where `kind` is `noop` / `human` / `llm` / `lsp` / `hybrid`. LLM labellers use a three-segment recipe `llm:<provider>:<model>` (e.g. `llm:deepseek:deepseek-chat`); LSP labellers two-segment `lsp:<resolver>` (e.g. `lsp:rust-analyzer`); human labellers two-segment `human:<id>` (e.g. `human:cferreir4`). Required on `schema_version: "2"` JSONL records; persists into the `edge_audit_history` table and is the join key for cross-labeller agreement views | `AUDIT-LABEL-SCHEMA.md` (schema v2) + `BACKLOG.md` § Priority 1 (g) + (i) + (j) + `gumiho-mudang-labeller/scope-audit-labeller-llm/README.md` |
...
| `provider-agnostic LLM wrapper` | Architecture of `scope-audit-labeller-llm`: a `Provider` trait (HTTP transport seam — one `complete(&Prompt)` method, plus stable `provider_id` and `model_id` strings) plus a generic `LlmLabeller<P: Provider>` that wraps any provider into a `Labeller`. Concrete providers (DeepSeek in sprint 0010; Anthropic / OpenAI / Gemini / local in follow-up sprints) live behind individual cargo features; the labeller, prompt rendering and verdict parsing are provider-independent. Adding a new provider is a new file under `src/providers/` and a feature row in `Cargo.toml`; no surgery on the labeller | `BACKLOG.md` § Priority 1 (b₂) + `gumiho-mudang-labeller/scope-audit-labeller-llm/README.md` § Architecture |
| `prompt template` | The system + user message pair `scope-audit-labeller-llm` sends to a provider per record. System prompt names the seven labeller-fillable verdict-object keys (`label`, `evidence`, `target_proposed`, `kind_proposed`, `confidence_proposed`, `reasoning_text`, `lang_version_evidence`) so verdict parsing is a direct `serde_json::from_str` rather than a translation step. User message exposes the edge under audit: `kind`, `from`, `to`, `extractor_confidence`, `producer`, `pattern_id`, `lang_version`, `source_snippet`. Rendered by `prompt::render_prompt`; the model is instructed to abstain (`label: null`) when uncertain | `gumiho-mudang-labeller/scope-audit-labeller-llm/README.md` § Prompt template + `src/prompt.rs` |
| `scope-audit-labeller-llm` | Crate in the labeller workspace implementing `Labeller` via an LLM provider. Houses the `Provider` trait, the `LlmLabeller<P>` wrapper, the prompt template, the `Verdict` parse target, an in-process `MockProvider` for tests, and concrete provider modules each behind their own cargo feature. First provider: DeepSeek (`deepseek` feature, default). Binary `scope-audit-labeller-llm` reads v2 JSONL from stdin and writes labelled v2 JSONL to stdout, ready for `scope audit confidence --label` | `BACKLOG.md` § Priority 1 (b₂) + `gumiho-mudang-labeller/scope-audit-labeller-llm/README.md` |
