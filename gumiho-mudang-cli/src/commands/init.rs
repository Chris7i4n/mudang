...
use gumiho_mudang_scope::config::project::vendor_patterns_for_languages;
...
pub fn run(args: &InitArgs, project_root: &Path) -> Result<()> {
    ...
    // Ruby detection
    let has_gemspec = std::fs::read_dir(project_root)?
        .filter_map(|e| e.ok())
        .any(|e| e.path().extension().and_then(|ext| ext.to_str()) == Some("gemspec"));
    if project_root.join("Gemfile").exists()
        || project_root.join("Rakefile").exists()
        || project_root.join("config.ru").exists()
        || project_root.join(".ruby-version").exists()
        || has_gemspec
    {
