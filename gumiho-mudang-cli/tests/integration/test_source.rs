/// Integration tests for `scope source`.
...
#[test]
fn test_source_prints_symbol_code() {
    let (_dir, root) = setup_indexed_fixture();
    Command::cargo_bin("mudang")
        .unwrap()
        .args(["source", "PaymentService"])
        .current_dir(&root)
        .assert()
        .success()
...
#[test]
fn test_source_json_has_standard_envelope() {
    let (_dir, root) = setup_indexed_fixture();
    let output = Command::cargo_bin("mudang")
        .unwrap()
        .args(["source", "PaymentService", "--json"])
        .current_dir(&root)
        .assert()
        .success()
        .get_output()
...
#[test]
fn test_source_unknown_symbol_fails() {
    let (_dir, root) = setup_indexed_fixture();
    Command::cargo_bin("mudang")
        .unwrap()
        .args(["source", "NoSuchSymbol"])
        .current_dir(&root)
        .assert()
        .failure()
