...
fn test_rust_enum_sketch_json_includes_signatures() {
    ...
    let has_sig = variants
        .iter()
        .any(|v| v["signature"].as_str().is_some_and(|s| s.contains("tx_id")));
...
/// Generic type parameters (`T`, `F`, `R`, ...) must not produce
/// `references_type` edges. They are not real type references — the Rust
/// grammar exposes them as `type_identifier` nodes alongside concrete types,
/// so the edge query has to filter them out post-capture.
///
/// Surfaced while indexing tokio in Sprint 12 — single-letter generics
/// accounted for ~24% of `references_type` edges.
#[test]
fn test_rust_generic_params_do_not_emit_type_refs() {
    let dir = TempDir::new().unwrap();
    std::fs::write(
        dir.path().join("Cargo.toml"),
        "[package]\nname = \"generic-refs\"\nversion = \"0.0.0\"\nedition = \"2021\"\n",
    ...
    // `F` and `T` are generic params on `pass` and `Holder::wrap`.
    // `String` is a real type whose `references_type` edge must survive.
    std::fs::write(
        dir.path().join("src/lib.rs"),
        "pub struct Holder { pub label: String }\n\nimpl Holder {\n    pub fn wrap<T>(item: T) -> T { item }\n}\n\npub fn pass<F>(f: F) -> F { f }\n",
    )
    .unwrap();
    ...
    let conn = rusqlite::Connection::open(dir.path().join(".scope/graph.db")).unwrap();

    for ghost in ["F", "T"] {
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM edges WHERE to_id = ?1 AND kind = 'references_type'",
        ...
        assert_eq!(
            count, 0,
            "generic param `{ghost}` must not produce references_type edges; got {count}"
        );
    }

    // Sanity: the real `String` reference is still indexed.
    let string_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM edges WHERE to_id = 'String' AND kind = 'references_type'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert!(
        string_count > 0,
        "real type `String` should still produce a references_type edge; got {string_count}"
    );
}

/// Regression: an `impl Foo { ... }` block that appears earlier in the source
/// than the `struct Foo;` declaration must not break indexing. Tree-sitter emits
/// captures in document order, so the impl's methods are pushed onto the symbol
/// vec before the struct is. After `associate_rust_impl_methods` rewrites each
/// method's `parent_id` to point at the struct, the symbols still need to be
/// inserted in an order that satisfies the `parent_id` FK on `symbols`.
///
/// Found while indexing tokio-1.52.3 in Sprint 12 — the index aborted with
/// `Error code 787: Foreign key constraint failed`.
#[test]
fn test_rust_impl_before_struct_does_not_break_index() {
    let dir = TempDir::new().unwrap();

    // Cargo.toml so the workspace looks Rust-shaped.
    std::fs::write(
        dir.path().join("Cargo.toml"),
        "[package]\nname = \"impl-before-struct\"\nversion = \"0.0.0\"\nedition = \"2021\"\n",
    )
    .unwrap();
    std::fs::create_dir_all(dir.path().join("src")).unwrap();
    // The impl block appears in the source BEFORE the struct it targets.
    std::fs::write(
        dir.path().join("src/lib.rs"),
    ...
    // Both methods exist as `method` symbols whose parent is `Widget`.
    let count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM symbols m
             JOIN symbols p ON m.parent_id = p.id
             WHERE m.kind = 'method' AND p.name = 'Widget' AND p.kind = 'struct'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(
        count, 2,
        "both `new` and `render` should be indexed as methods of Widget; got {count}"
    );
}
